...
import { columnsEntrepot } from "./ColumnsTable";

type Props = {
  data: Entrepot[];
  onView?: (Entrepot: Entrepot) => void;
  onEdit: (Entrepot: Entrepot) => void;
  onDelete: (Entrepot: Entrepot) => void;
};

export default function EntrepotTable({ data, onEdit, onDelete, onView }: Props) {
  const columns = useMemo(() => columnsEntrepot(), []);
  ...
  const table = useMaterialReactTable({
    ...
    renderRowActions: ({ row }: { row: { original: Entrepot } }) => (
      <div className="flex gap-2 items-center">
        {onView && (
          <button
            onClick={() => onView(row.original)}
            aria-label={`Voir l'entrepot ${row.original.nom}`}
        ...
        <button
          ...
          aria-label={`Modifier de l'entrepot ${row.original.nom}`}
        ...
        <button
          ...
          aria-label={`Supprimer de l'entrepot ${row.original.nom}`}
