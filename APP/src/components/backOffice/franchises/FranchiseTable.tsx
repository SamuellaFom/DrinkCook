...
import { columnsFranchise } from "./ColumnsFranchise";
...
export default function FranchiseTable({ data, onEdit, onDelete }: Props) {
  const columns = useMemo(() => columnsFranchise(), []);
  ...
  const table = useMaterialReactTable({
    ...
    renderRowActions: ({ row }: { row: { original: Franchise } }) => (
      <div className="flex gap-2 items-center">
        <button
          onClick={() => navigate(`/franchises/${row.original.id}`)}
          aria-label={`Voir la franchise ${row.original.nom}`}
        ...
        <button
          ...
          aria-label={`Modifier la franchise ${row.original.nom}`}
        ...
        <button
          ...
          aria-label={`Supprimer la franchise ${row.original.nom}`}
