...
import { columnsPanne } from "./ColunmsPanne";
import { useMemo } from "react";

type Props = {
  data: Panne[];
  onView?: (Panne: Panne) => void;
  onEdit: (Panne: Panne) => void;
  onDelete: (Panne: Panne) => void;
};

export default function PanneTable({ data, onEdit, onDelete, onView }: Props) {
  const columns = useMemo(() => columnsPanne(), []);
  ...
  const table = useMaterialReactTable({
    ...
    renderRowActions: ({ row }: { row: { original: Panne } }) => (
      <div className="flex gap-2 items-center">
        {onView && (
          <button
            onClick={() => onView(row.original)}
            aria-label={`Voir le panne ${row.original.id}`}
        ...
        <button
          ...
          aria-label={`Modifier de la panne ${row.original.id}`}
        ...
        <button
          ...
          aria-label={`Supprimer la panne ${row.original.id}`}
